    ...
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
                ...
                latInputDeg <= 90 && latInputDeg >= -90)
            {
                ...
                double pi = Math.PI;                                // pi as simplified constant
                double cosLatInput = Math.Cos(latInputRad);         // cosine of input latitude
                double sinLatInput = Math.Sin(latInputRad);         // sine of input latitude
                double sinLatInputSq = Math.Pow(sinLatInput, 2.0);  // sine of input latitude squared

                // Variable for square root function for E/W denominator
                double ewDenSquareRoot = Math.Sqrt((1.0 - (ESQD * sinLatInputSq)));

                // Variable for power function for N/S denominator
                double nsDenPower = Math.Pow((1.0 - (ESQD * sinLatInputSq)), (3.0/2.0));


                // East/West calculation (in meters)
                double ewCalcMeters = (pi * ERAD * cosLatInput) / (180.0 * ewDenSquareRoot);

                // North/South calculation (in meters)
                double nsCalcMeters = (pi * ERAD * (1.0 - ESQD)) / (180.0 * nsDenPower);
